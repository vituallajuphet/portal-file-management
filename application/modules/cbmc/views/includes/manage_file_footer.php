...
var myapp = new Vue({
	...
	data(){
		return {
			...
			check_user_id:[],
			archieved_files:[],
			departments:<?= json_encode(get_my_department());?>,
			selected_file_id:"",
			is_loading: false,
		...
		methods:{
			get_files(){
				...
				return new Promise((resolve, reject)=>{
					axios.get(`${self.base_url}api/get_files_cbmc`).then(res =>{
						let resp = res.data;
						if(resp.code == 200){
			...
			get_arc_files(){
				let self = this;
				return new Promise((resolve, reject)=>{
					axios.get(`${self.base_url}api/get_archieved_files`).then(res =>{
						let resp = res.data;
						if(resp.code == 200){
			...
			show_file_details(file_id){
				...
				axios.get(`${self.base_url}api/get_file_users/${file_id}`).then(res =>{
					let resp = res.data;
					if(resp.code == 200){
			...
			show_delete_file(file_id){
				...
				self.confirm_alert("Are you sure to delete this file?").then(res =>{
					...
					if(res == 200){
						axios.post(`${self.base_url}api/cbmc_delete_file`, formdata).then(res=> {
							let resp = res.data;
							self.is_loading = true;
							if(resp.code == 200){
			...
			show_delete_archieve(file_id){
				...
				self.confirm_alert("Are you sure to delete permanently this file?").then(res =>{
					self.is_loading = true;
					if(res == 200){
						axios.post(`${self.base_url}api/delete_archieve_file`, formdata).then(res=> {
							let resp = res.data;
							if(resp.code == 200){
			...
			show_restore_file(file_id){
				...
				self.confirm_alert("Are you sure to restore this file?").then(res =>{
					self.is_loading = true;
					if(res == 200){
						axios.post(`${self.base_url}api/restore_file`, formdata).then(res=> {
							let resp = res.data;
							self.is_loading = false;
							if(resp.code == 200){
			...
			show_restrict_user(){
				...
				let self = this;
				axios.get(`${self.base_url}api/get_restricted_user/${self.selected_restrict_file_id}`).then(res=>{
					let resp = res.data;
					if(resp.code==200){
				...
				if(selected_user_ids.length == 0 && selected_un_restrict_ids == 0){
					...
					self.confirm_alert(con_message).then(res =>{
						if(res == 200){
							...
							axios.post(`${self.base_url}api/restrict_users`, frmdata).then(res=>{
								let resp = res.data;
								self.is_loading = false;
								if(resp.code == 200){
